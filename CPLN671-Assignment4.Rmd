...
ggplot(khat_df, aes(x = r)) +
  geom_line(aes(y = iso, color = "Observed K(r)"), size = 1) +
  geom_line(aes(y = theo, color = "Theoretical K(r)"), linetype = "dashed", size = 1) +
  labs(
    x = "r (Distance)",
...
```{r k-function analysis adjusted, message=FALSE, warning=FALSE}
...
```{r better viz, message=FALSE, warning=FALSE}

# Adjust the observed and envelope values of the L-function to emphasize deviations from CSR.
L2 <- Lenv 
L2$obs <- L2$obs-L2$r
